...
app = FastAPI(
    title="CNC Bearing AI API", description="Predictive maintenance API", version="1.0"
)
...
def predict(bearing_unit: int = 1, contamination: float = 0.05):
    ...
    # Filtruj startup phase (pierwsze 10% cykli)
    startup_cutoff = int(len(df) * 0.1)
    anomalies_idx = anomalies_idx[anomalies_idx > startup_cutoff]
